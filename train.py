...
def eval(model, val_loader, device):
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            model.test() if hasattr(model, 'test') else model.forward()
    ...
    emo_f1_weighted = f1_score(total_emo_label, total_emo_pred, average='weighted', zero_division=0)
    emo_f1_unweighted = f1_score(total_emo_label, total_emo_pred, average='macro', zero_division=0)
...
def train_model(train_json, model, audio_path='', video_path='', max_len=5,
    ...
    best_emo_acc = 0.0
    best_emo_f1 = 0.0
    best_emo_f1_unweighted = 0.0
    best_emo_acc_unweighted = 0.0
    best_emo_epoch = 1
    best_emo_cm = []

    # 加入余弦退火学习率调度器
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(model.optimizer, T_max=num_epochs, eta_min=1e-6)
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            losses = model.get_current_losses() if hasattr(model, 'get_current_losses') else {'emo_CE': 0}
            total_loss += losses.get('emo_CE', 0)
        ...
        # 优化保存逻辑：优先根据 Macro F1 (Unweighted F1) 保存，兼顾 Weighted F1
        if emo_f1_unweighted > best_emo_f1_unweighted or (emo_f1_unweighted == best_emo_f1_unweighted and emo_f1_weighted > best_emo_f1):
            ...
            checkpoint = {
                ...
                'feature_config': {
                    'track': args.track_option,
                    'task': f'{args.labelcount}-class',
                    'audio_feature': args.audiofeature_method,
                    'video_feature': args.videofeature_method,
                    'use_personality': True,
                    'splitwindow_time': args.splitwindow_time,
                },
                'fold': None,
                'seed': seed,
            }
            torch.save(checkpoint, save_path)
            logger.info(f"--> Saved new best model (Epoch {best_emo_epoch}) based on Macro F1: {best_emo_f1_unweighted:.4f}")

        # 更新学习率
        scheduler.step()
        current_lr = scheduler.get_last_lr()[0]
        logger.info(f"Epoch {epoch + 1} ended. Current LR: {current_lr:.6e}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train Bi-CFNet Adapted Model on MPDD")
    ...
    parser.add_argument('--device', type=str, default='cpu', help="Device to train the model on, e.g. 'cuda' or 'cpu'")
    parser.add_argument('--seed', type=int, default=3407, help="Random seed for reproducibility")
    parser.add_argument('--model', type=str, default='our', help="PyTorch model name resolved by models.create_model")
    ...
    best_model_name = f"best_model_{time.strftime('%Y-%m-%d-%H.%M.%S', time.localtime())}.pth"

    logger.info(f"Bi-CFNet Adapted Model Training: splitwindow_time={args.splitwindow_time}, audio={args.audiofeature_method}, video={args.videofeature_method}")
