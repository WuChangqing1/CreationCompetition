...
class AudioVisualDataset(Dataset):
    def __init__(self, json_data, label_count, personalized_feature_file=None, max_len=10, batch_size=32,
                 audio_path='', video_path='', isTest=False, use_personality=True, personality_dim=1024):
        self.data = json_data
        ...
        self.personalized_features = (
            self.load_personalized_features(personalized_feature_file)
            if use_personality and personalized_feature_file
            else {}
        )
    ...
    def __getitem__(self, idx):
        ...
        # Preserve the exact subject token. Converting to int loses leading
        # zeroes and can break the id -> personality mapping.
        personalized_id = filename.split('_')[0]

        if self.use_personality and personalized_id in self.personalized_features:
            personalized_feature = torch.tensor(self.personalized_features[personalized_id], dtype=torch.float32)
        else:
            personalized_feature = torch.zeros(self.personality_dim, dtype=torch.float32)
